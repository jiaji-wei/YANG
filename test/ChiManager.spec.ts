...
describe('CHIManager', () => {
  ...
  describe('Mint CHI NFT', async () => {
    describe('success cases', () => {
      it('succeeds for mint', async () => {
        ...
        await expect(mint(gov.address, USDCAddress, USDTAddress, FeeAmount.MEDIUM, vaultFee, gov, proof)).to.be.revertedWith(
          'Non-existent pool'
        )
        // more than FEE_BASE
        await expect(mint(gov.address, token0.address, token1.address, FeeAmount.MEDIUM, 1e6, gov, proof)).to.be.revertedWith('f')

        const { tokenId: tokenId1 } = await mint(gov.address, token0.address, token1.address, FeeAmount.MEDIUM, vaultFee, gov, proof);
    ...
    describe('fails cases', () => {
      it('fails if minter is not a gov', async () => {
        ...
        const proof = info.whitelist[gov.address].proof;
        await expect(
          mint(gov.address, token0.address, token1.address, FeeAmount.MEDIUM, vaultFee, wallets[1], proof)
  ...
  describe('Manage CHI', async () => {
    ...
    beforeEach('Mint CHI', async () => {
      ...
      const gov = govs[0];
      const proof = info.whitelist[gov.address].proof;
      const { tokenId, vault } = await mint(gov.address, token0.address, token1.address, FeeAmount.MEDIUM, vaultFee, gov, proof)
      tokenId1 = tokenId
      ...
      let _yangId = await yangNFT.connect(gov).callStatic.mint(trader.address, proof);
